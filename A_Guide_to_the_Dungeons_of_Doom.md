...
| **f** | 战斗到死 | 后跟一个方向，表示和指定方向丢生物打架，直到你或它死 |
...
| **r** | | read. 读你带的一个卷轴 |
| **e** | | eat. 吃掉背包里的食物 |
| **w** | 挥武器 | wield. 从背包中取出武器然后带着它战斗，替换掉你正在用的那个 |
...
| **d** | 丢掉 | Drop. 丢掉背包里的某个东西，放在地板上。一个空间里只能放一个东西。你不能丢掉一个被诅咒的东西，如果你在挥它或戴它。 |
| **c** | 起名 | call. 背包里的某个东西，如果你想记下点什么，你可以使用“起名”命令。 |
| **o** | | options. 检查和设置选项。在“选项”部分会详细介绍这个命令。 |
| **^R** | redraw. 重绘屏幕 | 当谬误信息或传输误差搞乱屏幕时有用。 |
| **^P** | print. 打印上条消息 | 如果消息消失了，但你没看清楚时，有用。连续重复使用会显示最近 5 条消息。 |
| <ESC> |  | 取消命令、前缀、数量。 |
| **!** |  | 离开到 shell，以便执行某些命令。 |
| **Q** | 退出 | Quit. 退出游戏。 |
| **S** |  | 保存当前游戏到一个文件。它会问你，默认情况下你想把文件保存在哪里。警告：rogue 不会让你启动以保存游戏到备份，并且他会移除保存的文件，一旦你启动了已恢复的游戏。这是为了阻止人们在危险前保存游戏，并在死后重新开始。要恢复一个保存的游戏，需要传文件名（作为参数）给 rogue。比如 `$ rogue save_file` |
| **v** | | 打印程序版本号。 |
| **)** | | 打印你当前使用的武器。 |
| **]** | | 打印你当前穿着的护甲。 |
| **=** | | 打印你当前戴着的戒指。 |
...
许多可操作物品的命令会提示你指定哪个物品将被操作。
如果你改变心意并且不想执行那个命令，只需要按 <ESC> 然后命令会终止。

一些物品，像护甲和武器，是有区分的。
别的，像卷轴和魔药会给出标签（labels）（根据种类有所区分）。
在一场游戏中，两个同种（the same kind）、同标签的物品是同一类（the same type）的。
但是，下一次游戏游戏就不一样了。

当你使用这些贴标签的物品时，如果它的效果是显而易见的。
魔药和卷轴就变得确定在这时，其他东西就不会了。
你或许想给这些东西起名，以便你后面能认得它，你可以使用 c 命令（见上）。

### 7.1 武器
一些武器，像箭，一次会出现多个，其他的一次只一个。
想使用武器，你必须装备上它。
想从弓里射出一支箭，你必须先装备弓，然后丢（drop）出箭。
同一时间只能装备一个武器，但如果正装备的武器被诅咒了，你就不能换武器。
...
棍和杖的作用是一样的。
棍是用一种木材来识别的；杖是用金属或骨头。
他们通常是你想对远距离的某东西做点什么，那么你可以先指向你想做的。
但是，一些棍不受指向影响。
棍带有多种魔法，数量是随机的，用完后，就变成了木头或金属。
